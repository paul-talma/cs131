...
class UnsafeMemory {
  public static void main(String args[]) {
    ...
    try {
      boolean virtual;
      if (args[0].equals("Platform")) virtual = false;
      else if (args[0].equals("Virtual")) virtual = true;
      else throw new Exception(args[0]);

      var nValues = (int) argInt(args[1], 0, Integer.MAX_VALUE);

      State s;
      if (args[2].equals("Null")) s = new NullState(nValues);
      else if (args[2].equals("Synchronized")) s = new SynchronizedState(nValues);
      else if (args[2].equals("Unsynchronized")) s = new UnsynchronizedState(nValues);
      else if (args[2].equals("AcmeSafe")) s = new AcmeSafeState(nValues);
      else throw new Exception(args[2]);
  ...
  private static void dowork(boolean virtual, int nThreads, long nTransitions, State s)
  ...
  private static void test(long[] output) {
    ...
    if (osum != 0) error("output sum mismatch", osum, 0);
  }
